...
object LapCsv {
    ...
    fun build(lap: RecordedLap, carName: String?): String = buildString {
        ...
        appendLine("pos_x,pos_z,lat_g,long_g,clutch_pct,tyre_fl_c,tyre_fr_c,tyre_rl_c,tyre_rr_c")
        val temps = lap.hasTyreTemps
        for (i in 0 until lap.sampleCount) {
            val steer = lap.steerDeg[i]
            append("%.3f,%.1f,%.0f,%.0f,%s,%.0f,%.0f,%.2f,%.2f,%.3f,%.3f,%.0f".fmt(
                ...
                lap.posX[i].toDouble(), lap.posZ[i].toDouble(),
